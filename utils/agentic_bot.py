...
class AgenticCommerceBot:
    ...
    def _prod_dict(p):
        return {
            'id': p.id, 'name': p.name, 'price': float(p.price), 'formatted_price': f"Rs. {p.price:.2f}",
            'category': p.category.name if p.category else 'General',
            'image_url': p.product_image.url if p.product_image else '/static/images/default.jpg',
            'stock': p.stock, 'url': f"/products/{p.id}/"
        }
    ...
    def process_query(message: str, request) -> dict:
        ...
        res = {'reply': '', 'action_type': 'general', 'products': [], 'cart_data': {'count': cart.total_items_count, 'total': cart.total_price}, 'quick_replies': ['Explore Store', 'Active Auctions', 'My Cart']}
        ...
        if any(low.startswith(w) for w in ['add ', 'buy ']) or 'to cart' in low:
            ...
            p = Product.objects.filter(status=True, is_approved=True).filter(Q(name__icontains=target) | Q(description__icontains=target) | Q(category__name__icontains=target)).first() if target else None
            if not p:
                ...
                if not created: it.quantity = min(it.quantity + 1, p.stock or 1); it.save()
                res.update({'reply': f"Added **{p.name}** (Rs. {p.price:.2f}) directly to your cart!", 'action_type': 'cart_updated', 'products': [AgenticCommerceBot._prod_dict(p)], 'cart_data': {'count': cart.total_items_count, 'total': cart.total_price}, 'quick_replies': ['Proceed to Checkout', 'View Cart', 'Continue Shopping']})
            return res
        ...
        if any(w in low for w in ['show cart', 'view cart', 'my cart', 'check cart', 'cart summary', 'what is in my cart', 'open cart']):
            ...
            if not items.exists():
                ...
                res.update({'reply': f"Here is your cart (**{cart.total_items_count} items**):\n\n" + "\n".join(lines) + f"\n\n**Total: Rs. {cart.total_price:.2f}**", 'action_type': 'cart_summary', 'products': [AgenticCommerceBot._prod_dict(i.product) for i in items], 'quick_replies': ['Proceed to Checkout', 'Clear Cart', 'Continue Shopping']})
            return res
        ...
        if any(w in low for w in ['checkout', 'proceed to pay', 'buy all', 'place order', 'go to checkout']):
            if cart.total_items_count == 0:
                ...
                res.update({'reply': f"You have **{cart.total_items_count} item(s)** totaling **Rs. {cart.total_price:.2f}** ready for checkout.", 'action_type': 'checkout_redirect', 'redirect_url': '/checkout/', 'quick_replies': ['View Cart', 'Continue Shopping']})
            return res
        ...
        if any(w in low for w in ['auction', 'bid', 'bidding', 'flash auction', 'ending soon']):
            ...
            if not auctions:
                ...
                res.update({'reply': "\n\n".join(lines), 'action_type': 'auction_list', 'products': [AgenticCommerceBot._prod_dict(a.product) for a in auctions if a.product], 'quick_replies': ['Explore Store', 'My Cart']})
            return res
        ...
        if any(w in low for w in ['pickup', 'location', 'where', 'block', 'kumari hall', 'skill block', 'nepal block', 'timing', 'hours', 'contact']):
            res.update({'reply': "📍 **Islington Pickup Spots:** Kumari Hall, Skill Block, Nepal Block, Alumini Block, Brit House & Himal Block.\n\n⏰ **Hours:** 10:00 AM – 4:00 PM (Mon–Fri)\n💳 **Payment:** Cash on Campus (COD) or eSewa Sandbox.\n📧 **Help:** info@islingtonmarket.np | +977-1-4412345"})
            return res
        ...
        if cat_match: qs = qs.filter(category=cat_match)
        ...
        if any(w in low for w in ['cheapest', 'lowest price', 'affordable']): qs = qs.order_by('price')
        ...
        if not results:
            ...
            res.update({'reply': f"Found **{len(results)} item(s)**{f' under Rs. {max_p:.2f}' if max_p else ''}{f' in *{cat_match.name}*' if cat_match else ''}:", 'action_type': 'product_list', 'products': [AgenticCommerceBot._prod_dict(p) for p in results], 'quick_replies': [f"Add {first_n[:18]} to cart", 'View Cart', 'Checkout']})
        return res
