...
class EmailMicroservice:
    ...
    def _send(cls, recipients, subject, html):
        ...
        # Strategy 1: Direct SSL SMTP to Gmail (Port 465)
        try:
            ...
            logger.info(f"Email successfully sent via SSL to {r}")
            return True
        except Exception as e:
            logger.warning(f"SMTP_SSL failed ({e}), trying TLS (587) fallback...")

        # Strategy 2: TLS SMTP to Gmail (Port 587)
        try:
            m = MIMEMultipart('alternative')
            m['Subject'] = subject
            m['From'] = from_hdr
            ...
            with smtplib.SMTP('smtp.gmail.com', 587, timeout=12) as s:
                ...
                s.login(user, pwd)
                s.send_message(m, from_addr=user, to_addrs=r)
            logger.info(f"Email successfully sent via TLS to {r}")
            return True
        except Exception as e:
            logger.warning(f"SMTP TLS failed ({e}), trying Django EmailMultiAlternatives fallback...")
        ...
        try:
            ...
            logger.error(f"All email dispatch methods failed for {r}: {e}")
            return False

    @classmethod
    def send_sync(cls, to, sub, html):
        """Sends email synchronously to guarantee delivery before WSGI process recycling."""
        return cls._send(to, sub, html)

    @classmethod
    def send_async(cls, to, sub, html):
        """Sends email synchronously for critical activation emails to prevent worker process drops."""
        return cls._send(to, sub, html)
    ...
    def send_otp_email(cls, u, code):
        ...
        return cls.send_sync(u.email, f"Your Verification Code: {code}", cls._wrap("Account Verification", "Security OTP", b))
    ...
    def send_activation_email(cls, u, activation_url):
        ...
        return cls.send_sync(u.email, "Activate Your Islington Marketplace Account", cls._wrap("Account Verification", "Email Verification Link", b))
    ...
    def send_order_confirmation_email(cls, order, site_url=""):
        ...
        return cls.send_sync(order.buyer_email, f"Order Confirmed #{order.id}", cls._wrap(f"Order #{order.id} Confirmed", "Order Receipt", b))
    ...
    def send_seller_new_order_email(cls, seller, product, order, qty=1, site_url=""):
        ...
        return cls.send_sync(seller.email, f"New Order for {product.name}!", cls._wrap("New Sale Alert", "Student Sale", b))
    ...
    def send_product_approved_email(cls, seller, product, site_url=""):
        ...
        return cls.send_sync(seller.email, f"Listing Approved: {product.name}", cls._wrap("Listing Approved", "Item Live", b))
    ...
    def send_product_rejected_email(cls, seller, product, reason=""):
        ...
        return cls.send_sync(seller.email, f"Listing Update: {product.name}", cls._wrap("Listing Update", "Moderation Notice", b))
    ...
    def send_admin_new_pending_review_email(cls, product, seller, admin_url=""):
        ...
        return cls.send_sync(emails, f"Review Required: {product.name}", cls._wrap("Moderation Notice", "Admin Action", b))
